...
*26 projects. Tools that notify you when agents finish, fail, pause, or wait for approval.*
...
*40 projects. Review panes, diff browsers, file viewers, and tools for returning structured feedback to an agent.*
...
*92 projects. Keybindings, selectors, launchers, and layout helpers for moving quickly and safely through panes.*
...
*22 projects. Temporary or compact displays that summarize which agents are active, blocked, idle, or finished.*
...
- **[Herdr website](https://herdr.dev/):** Installation, concepts, supported agents, remote access, and introductory socket examples.
- **[Configuration reference](https://github.com/ogulcancelik/herdr/blob/master/CONFIGURATION.md):** Configuration files, live reload, keybindings, commands, themes, notifications, and scrollback.
- **[Socket API reference](https://github.com/ogulcancelik/herdr/blob/master/SOCKET_API.md):** The newline-delimited JSON protocol, request envelopes, events, layout methods, agent methods, reads, waits, and input.
- **[Integration reference](https://github.com/ogulcancelik/herdr/blob/master/INTEGRATIONS.md):** Built-in support and hook paths for Pi, Claude Code, Codex, and OpenCode.
- **[Official Agent Skill](https://github.com/ogulcancelik/herdr/blob/master/SKILL.md):** Instructions for agents that are already running inside Herdr.
- **[Herdr releases](https://github.com/ogulcancelik/herdr/releases):** Current versions, release notes, and compatibility changes.

### Guides from the original catalog

- **[Configuration guide](https://github.com/yigitkonur/awesome-herdr/blob/main/docs/configuration.md):** Config locations, reload behavior, keybindings, themes, interface options, notifications, and logs.
- **[Socket API guide](https://github.com/yigitkonur/awesome-herdr/blob/main/docs/socket-api.md):** Protocol structure, method groups, reads, waits, events, and a client-library checklist.
- **[Integrations guide](https://github.com/yigitkonur/awesome-herdr/blob/main/docs/integrations.md):** Built-in integrations, hooks, environment variables, and a checklist for custom integrations.
- **[Agent workflows guide](https://github.com/yigitkonur/awesome-herdr/blob/main/docs/agent-workflows.md):** Worker panes, pair programming, watcher panes, wait commands, and safety guidance.

## Herdr reference

Most projects in this list build on a small group of Herdr concepts:

- `workspace.*`, `tab.*`, and `pane.*` methods create, inspect, and change the terminal layout.
- `agent.*` methods work with terminal-backed coding agents and their reported state.
- `events.subscribe` streams status, output, focus, and layout changes to clients.
- `~/.config/herdr/config.toml` controls keys, indexed shortcuts, custom commands, interface behavior, notifications, and sounds.
- `herdr integration install <pi|claude|codex|opencode>` installs a supported agent integration.

Herdr changes quickly. Check the current Herdr release and each project's requirements before installing an integration.

### Common ways to build an integration

- Start with the configuration file for a keymap, theme, launcher, or status display.
- Use the socket API for a client library, service, dashboard, notification bridge, or hardware controller.
- Report editor or agent state through the appropriate pane and agent methods.
- Wrap selected operations in an MCP server when another AI client needs standard tools.
- Add a skill file when the main goal is to teach an agent how to use existing Herdr commands.
- Build an orchestrator from pane creation, command execution, status waits, and event subscriptions.

## About this edition

This is a plain-English rewrite of [yigitkonur/awesome-herdr](https://github.com/yigitkonur/awesome-herdr). It preserves all 842 project links from the source snapshot retrieved on August 16, 2026, while removing badges, repeated navigation links, duplicated boilerplate, and promotional wording. The upstream catalog is available under the [MIT License](https://github.com/yigitkonur/awesome-herdr/blob/main/LICENSE).
